...
class StackRGGCN(Layer):
    ...
    def _call(self, inputs):
        ...
        for i in range(len(self.E_start)):
            ...
            # x1 = tf.add_n([dot(self.E_end[i], Vix, sparse=True), dot(self.E_start[i], Vjx, sparse=True), self.bv2[i]])
            # x1 = tf.nn.sigmoid(x1)
            # Uix = dot(x, self.Ui2[i])
            # x2 = dot(self.E_start[i], Uix, sparse=True)
            # Ujx = dot(x, self.Uj2[i])
            # x = tf.add_n([Ujx, dot(self.E_end[i].T, tf.multiply(x1, x2), sparse=True), self.bu2[i]])
            # x = tf.layers.batch_normalization(x)
            outputs.append(x)
